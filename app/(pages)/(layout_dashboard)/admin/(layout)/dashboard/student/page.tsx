...
export default function StudentManageDashboard() {
            ...
            cell: ({ cell }) => {
                return <div className="flex items-center justify-center">
                    <button type="button" className="mr-[10px] text-blue-400"
                        onClick={() => {
                            setModal({ ...modal, [`edit_student`]: true })
                    ...
                    <button type="button" className=" text-red-500">
                        <TrashIcon className="w-6 h-6" onClick={() => {
                            setModal({ ...modal, [`delete_student`]: true })
                            setSelectedStudent(cell.row.original)
                        }} />
                    </button>
    ...
    return (
        <div className='w-full'>
            <ToastContainer />
            <>
                <Modal show={modal[`delete_student`] || false} size="md" onClose={() => setModal({ ...modal, [`delete_student`]: false })} popup>
                    <Modal.Header />
                    <Modal.Body>
                        <form className="space-y-6" onSubmit={async (e) => {
                            ...
                            <div className="flex justify-center gap-4">
                                ...
                                <Button color="gray" onClick={() => {
                                    setModal({ ...modal, [`delete_student`]: false })
                                }}>
                                    Hủy
                                </Button>
                            </div>
                ...
                <Modal show={modal[`edit_student`] || false} size="xl" onClose={() => setModal({ ...modal, [`edit_student`]: false })} popup>
                    <Modal.Header />
                    <Modal.Body>
                        <form className="space-y-6" onSubmit={handleSubmit(async (data: any) => {
                            ...
                            setChange(!change)
                            reset()
                            setModal({ ...modal, [`edit_student`]: false })
                        })}>

                            <h3 className="text-xl font-medium text-gray-900 dark:text-white">Sửa học sinh</h3>
