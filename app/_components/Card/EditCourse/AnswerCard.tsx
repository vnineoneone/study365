...
export const AnswerCard = ({ indexChapter, indexTopic, hanldeForm, indexQuestion, setModal, modal, topic, quesiton, setImage, image }: any) => {
    ...
    const { fields: fieldsAnswer, append: appendAnswer, remove: removeAnswer } = useFieldArray({
        control,
        name: `chapters.${indexChapter}.topics.${indexTopic}.exam.data.questions.${indexQuestion}.answers`
    });

    console.log(fieldsAnswer, 3);
                    ...
                    return (
                        <div key={field.id} className='relative border-[1px] border-[#ececec] p-3 rounded-[10px] mb-10'>
                            <div className='mb-5'>
                                ...
                                <CustomCKEditor className="h-50" setValue={setValue} value={getValues().chapters[indexChapter]?.topics[indexTopic]?.exam?.data?.questions[indexQuestion]?.answers[indexAnswer]?.content_text} position={`chapters.${indexChapter}.topics.${indexTopic}.exam.data.questions.${indexQuestion}.answers.${indexAnswer}.content_text`} />
                                        ...
                                        process: (fieldName, file, metadata, load, error, progress, abort, transfer, options) => {
                                            ...
                                            request.onload = function () {
                                                if (request.status >= 200 && request.status < 300) {
                                                    setImage({ ...image, [`${getValues().chapters[indexChapter]?.topics[indexTopic]?.exam?.data?.questions[indexQuestion]?.answers[indexAnswer]?.id}`]: JSON.parse(request.response).url });
                            ...
                            <div className=' mb-2'>
                                <div className="flex-1 flex items-center ">
                                    <input {...register(`chapters.${indexChapter}.topics.${indexTopic}.exam.data.questions.${indexQuestion}.answers.${indexAnswer}.is_correct`)}
