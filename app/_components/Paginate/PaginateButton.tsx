...
export default function PaginateButton({ countPaginate, currentPage, setCurrentPage }: any) {
    const listPaginate = Array.from({ length: countPaginate }, (_, i) => i + 1);
    return (
        <div className=''>
            {
                countPaginate > 1 ?
                    <div className="flex justify-center items-center pt-10 pb-5">
                        <nav aria-label="Page navigation example">
                            <ul className="flex items-center -space-x-px h-8 text-sm">
                                ...
                                {
                                    listPaginate.map((l: number) => {
                                        return (
                                            <div key={l}>
                                                <button onClick={() => setCurrentPage(l)} className={`flex items-center justify-center px-3 h-8 leading-tight ${currentPage == l ? 'text-blue-600 border border-blue-300 bg-blue-50 hover:bg-blue-100 hover:text-blue-700' : 'text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700'} `}>{l}</button>
                                            </div>
                                        )
                                    })
                                }
